...
function load_and_run_sim(domain_path, problem_path, problem_constraint_fn, problem_constraint_args)
    vis = Visualizer()
    ...
    anim = Animation()
    ts = [0]
    @time plan = mip_planner(domain_path, problem_path, problem_constraint_fn, problem_constraint_args)
